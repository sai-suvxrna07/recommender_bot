...
YOUTUBE_API_KEY = os.getenv('YOUTUBE_API_KEY')  # Ensure to set this in your .env file
...
class Recommend(commands.Cog, name="recommend"):
    ...
    @commands.hybrid_command(
        name="trailer",
        description="Get the trailer for a movie enter the title.",
    )
    async def trailer(self, ctx: commands.Context, partial_movie_title: str):
        # Find the closest match for the movie title
        closest_match = process.extractOne(partial_movie_title, self.movie_titles.keys(), score_cutoff=70)
        if not closest_match:
            await ctx.send("No close match found for the movie title. Please try again.")
            return
        ...
        params = {
            ...
            'type': 'video',
            'key': YOUTUBE_API_KEY,  # Use environment variable for API key
            'maxResults': 1
        }
        ...
        if response.status_code == 200:
            ...
            if items:
                ...
                await ctx.send(f"Here is the link to the trailer for '{movie_title}': {video_url}")
            else:
                await ctx.send("No trailer found.")
        else:
            await ctx.send(f"Error: {response.status_code}")

    @commands.hybrid_command(
        name="chat",
        description="Talk with Google Gemini itself to receive a more tailored movie recommendation experience. (Use Quotes!!!)",
    )
